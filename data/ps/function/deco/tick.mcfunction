...
execute unless entity @a[tag=deco_user] run return -1

execute store result score #deco_pickslot ps store result storage ps:tmp deco_sel_info.id int 1 run scoreboard players get @p[tag=deco_user,distance=..10] ps_selecting_slot
